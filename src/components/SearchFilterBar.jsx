...
export default function SearchFilterBar({ 
  ...
  return (
    <div className="p-5 sm:p-7 rounded-3xl glass-panel-gold shadow-2xl text-left animate-slide-up border border-gold-primary/35">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 mb-5 border-b border-white/10">
        <div className="inline-flex glass-pill p-1 rounded-xl border border-white/15">
          <button 
            type="button"
            className={`px-3.5 sm:px-5 py-1.5 rounded-lg text-xs font-semibold tracking-wide transition-all cursor-pointer ${
              filters.status === 'All' 
                ? 'gold-gradient text-[#0a0c10] shadow-md shadow-gold-primary/35 font-bold border border-white/20' 
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-300 glass-pill px-3 py-1.5 rounded-full">
          <span className="bg-gold-primary/25 text-gold-primary border border-gold-primary/50 font-bold px-2 py-0.5 rounded-full text-xs shadow-sm shadow-gold-primary/20">
            {resultCount}
          </span>
          <span className="font-medium">Residences Matched</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            {CITIES.map(city => (
              <option key={city} value={city} className="bg-[#0b0e14] text-white">{city}</option>
            ))}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            {PROPERTY_TYPES.map(type => (
              <option key={type} value={type} className="bg-[#0b0e14] text-white">{type}</option>
            ))}
          </select>
        </div>
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-all text-center cursor-pointer ${
                  filters.minBeds === num 
                    ...
                    : 'glass-input text-zinc-400 hover:text-white hover:border-white/25'
                }`}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
        <div className="flex-1 min-w-[260px] relative flex items-center">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 glass-input rounded-xl text-white text-xs sm:text-sm focus:border-gold-primary outline-none transition-colors placeholder:text-zinc-500"
          />
        </div>
        ...
        {(filters.city !== 'All Locations' || 
          ...
          <button 
            ...
            className="flex items-center gap-1.5 text-xs text-zinc-300 hover:text-gold-primary px-4 py-2.5 rounded-xl glass-btn transition-all cursor-pointer"
          >
            <RotateCcw size={13} />
